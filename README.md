...
Parsing runs in a Web Worker to avoid blocking the UI. PDFs are extracted as structured Markdown (headings, lists, tables) so they can flow through the same chunker as native `.md` files. Extracted text is then run through a `sanitize` pass that strips web/wiki chrome, escape sequences, and other boilerplate before chunking.
