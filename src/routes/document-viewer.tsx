...
export function DocumentViewerPage() {
  const { documentId } = useParams<{ libraryId: string; documentId: string }>()
  ...
  useEffect(() => {
    async function loadDocument() {
      ...
      try {
        ...
        // Load chunk text for highlighting
        if (highlightChunkIndex !== null) {
          const chunk = await db.chunks
            .where('[libraryId+documentId]')
            .equals([docMeta.libraryId, documentId])
            .filter((c) => c.chunkIndex === highlightChunkIndex)
            .first()
...
function HighlightedText({ text, highlight, highlightRef }: HighlightedTextProps) {
  if (!highlight) {
  ...
  return (
    <div className="whitespace-pre-wrap text-gray-800">
      {before}
      <mark ref={highlightRef} className="bg-yellow-200 rounded px-0.5">{highlighted}</mark>
      {after}
    </div>
  )
}
